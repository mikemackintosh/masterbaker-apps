...
unless node['apps']['1password']['license_data'].nil?
  ...
  mac_os_x_userdefaults "1Password :: Keychain Path (#{node['apps']['1password']['settings']['keychain_path']})" do
    domain "ws.agile.1Password"
    key "AgileKeychainLocation"
    value node['apps']['1password']['settings']['keychain_path']
  end

  mac_os_x_userdefaults "1Password :: Lock keychain after #{node['apps']['1password']['settings']['keychain_lock_mins']} minutes" do
    domain "ws.agile.1Password"
    key "AgileKeychainLockTimeout"
    value node['apps']['1password']['settings']['keychain_lock_mins']
  end

  mac_os_x_userdefaults "1Password :: Look for updates every #{node['apps']['1password']['settings']['update_frequency_days']} days" do
    domain "ws.agile.1Password"
    key "AGUpdateFrequency"
    value node['apps']['1password']['settings']['update_frequency_days']
  end

  mac_os_x_userdefaults "1Password :: When checking for updates, do we accept beta code?" do
    domain "ws.agile.1Password"
    key "AGUpdateAllowBetas"
    value node['apps']['1password']['settings']['allow_beta_updates']
  end

  mac_os_x_userdefaults "1Password :: Backup Path (#{node['apps']['1password']['settings']['backup_path']})" do
    domain "ws.agile.1Password"
    key "keychainBackupFolderPath"
    value node['apps']['1password']['settings']['backup_path']
  end

  mac_os_x_userdefaults "1Password :: Retain backups for #{node['apps']['1password']['settings']['backup_retain_days']} days" do
    domain "ws.agile.1Password"
    key "keychainRotationFrequency"
    value node['apps']['1password']['settings']['backup_retain_days']
  end

  mac_os_x_userdefaults "1Password :: Retain data on clipboard for #{node['apps']['1password']['settings']['clipboard_clear_secs']} seconds" do
    domain "ws.agile.1Password"
    key "PasteboardClearTimeout"
    value node['apps']['1password']['settings']['clipboard_clear_secs']
  end

  mac_os_x_userdefaults "1Password :: Disable animations when locking and unlocking the keychain" do
    domain "ws.agile.1Password"
